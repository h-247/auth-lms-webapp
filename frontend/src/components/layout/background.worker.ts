 ...
 *
 * Optimisations applied (v2 – lightweight edition):
 *   • 30 fps cap via delta-time gate (was unbounded rAF = 60fps)
 *   • Desktop stars: 200 max (was 500) → pairMask 40K (was 250K)
 *   • Mobile stars:  80  max (was 150)
 *   • Nebulae: 4 (was 6) – fewer scale-transform calls per frame
 *   • Constellation distance: 90 px (was 110) – quadratically fewer line pairs
 *   • Pulse/traveling-dot system removed – eliminated heap churn
 *   • Pause message: cancels rAF when tab is hidden (CPU → ~0%)
 *   • Resume message: restarts rAF when tab becomes visible
 */
...
let rafHandle = 0; // track the current rAF id so we can cancel it

// ── 30fps gate ──────────────────────────────────────────────────────────────
const TARGET_INTERVAL = 1000 / 30; // ~33.33 ms
let lastFrameTime = 0;

// ── Star data ────────────────────────────────────────────────────────────────
// Fields per star: x, y, z, size, baseAlpha, twinkleSpeed, twinklePhase, hue, colorType, shimmer, initDelay
let starsData: Float32Array;
...
const CONSTELL_DIST = 90; // reduced from 110 → fewer pairs
let gridHead: Int32Array;
let gridNext: Int32Array;
let gridCols = 0;
let gridRows = 0;

// Constellation pair dedup – generation counter, no per-frame fill()
...
function createSprites() {
  ...
  // 2. Nebula Sprites (4 hues – matching the 4 nebulae we now draw)
...
function initStars(w: number, h: number) {
  ...
  // ↓ Reduced star counts: 200 desktop (was 500), 80 mobile (was 150)
  const maxStars = isMobile ? 80 : 200;
  starCount = Math.min(maxStars, Math.max(40, Math.round((w * h) / 5000)));
  ...
  // pairMask: 200×200 = 40 K entries (was 500×500 = 250 K)
  pairMask = new Uint16Array(starCount * starCount);
  connectedBuf = new Int32Array(starCount);

  // ↓ 4 nebulae (was 6) – removed 2 less-prominent ones
  const fixedNebulae = [
  ...
  // 1 shooting star always (was 3 on desktop)
  shootingStars = [{ x: 0, y: 0, vx: 0, vy: 0, life: 0, maxLife: 0, active: false }];
}
...
function update(t: number) {
  ...
  // Shooting star
  if (t - lastShootTime > (config.isDark ? 5 : 8) + Math.random() * 5) {
  ...
  for (const s of shootingStars) {
    ...
    // Pulse nearby stars (shimmer boost)
  ...
  // Decay shimmer
  for (let i = 0; i < starCount; i++) {
    starsData[i * STAR_STRIDE + 9] *= 0.97;
  }
}
...
function draw(timestamp: number) {
  ...
  // Snap to exact interval boundary to avoid drift
  ...
  const t = (timestamp - initTime) * 0.001;
              ...
              while (j !== -1) {
                if (i < j && starsData[j * STAR_STRIDE + 2] >= 0.45) {
                  ...
                  if (pairMask[maskIdx] !== pairGeneration) {
                    ...
                    const offI = i * STAR_STRIDE, offJ = j * STAR_STRIDE;
                    ...
                    const dx = six - sjx, dy = siy - sjy;
                    if (dx * dx + dy * dy < constellDistSq) {
                      ctx.moveTo(six, siy);
                      ctx.lineTo(sjx, sjy);
                    }
                  }
  ...
  if (mouse.x > 0 && mouse.y > 0) {
      ...
      for (let nx = Math.max(0, mgx - 2); nx <= Math.min(gridCols - 1, mgx + 2); nx++) {
        ...
        while (i !== -1) {
          ...
          if (z > 0.15) {
            const sx = starsData[off + 0] + mxOffset * z * 0.6;
            const sy = starsData[off + 1] + myOffset * z * 0.6;
            const dx = sx - mouseFollow.x, dy = sy - mouseFollow.y;
            ...
            if (bloomFactor >= 0.2 && dSq < grabDistSq) {
              if (connectedLen < connectedBuf.length) connectedBuf[connectedLen++] = i;
    ...
    // Sparse web between grabbed stars
    ctx.beginPath();
    ...
    for (let i = 0; i < connectedLen; i++) {
      ...
      for (let j = i + 1; j < connectedLen && connections < 2; j++) {
        ...
        const six = starsData[offI + 0] + mxOffset * zI * 0.6;
        const siy = starsData[offI + 1] + myOffset * zI * 0.6;
        const sjx = starsData[offJ + 0] + mxOffset * zJ * 0.6;
        const sjy = starsData[offJ + 1] + myOffset * zJ * 0.6;
        const dx = six - sjx, dy = siy - sjy;

        if (dx * dx + dy * dy < (grabDist * 0.8) ** 2) {
          ctx.moveTo(six, siy);
          ctx.lineTo(sjx, sjy);
          connections++;
        }
  ...
  for (let i = 0; i < starCount; i++) {
    ...
    let alpha = starsData[off + 4] * (z > 0.85
      ? (0.55 + 0.45 * twinkle)
      : (0.5 + 0.5 * twinkle));
    ...
    if (starSprites) {
      ...
      ctx.drawImage(
        ...
        sx - SPRITE_SIZE / 2, sy - SPRITE_SIZE / 2, SPRITE_SIZE, SPRITE_SIZE,
      );
      ctx.globalAlpha = 1.0;
    } else {
      ctx.beginPath();
      ctx.arc(sx, sy, starsData[off + 3], 0, Math.PI * 2);
      ctx.fillStyle = `hsla(${starsData[off + 7]}, 30%, 95%, ${alpha})`;
      ctx.fill();
    }
  }
...
self.onmessage = (e) => {
  ...
  switch (type) {
    ...
    case 'pause':
      // Tab became hidden – stop rAF to bring CPU to ~0%
      if (animationRunning) {
        animationRunning = false;
    ...
    case 'resume':
      // Tab became visible again – restart the animation loop
      if (!animationRunning && ctx) {
        animationRunning = true;
        lastFrameTime = 0; // reset so first frame isn't skipped
        rafHandle = requestAnimationFrame(draw);
      }
      break;
  }
