...
export function MicroQuizHistoryModal({ courseId, onClose, onSelectJob }: Props) {
  ...
  useEffect(() => {
    ...
    const fetchJobs = async () => {
      try {
        const data = await microQuizService.listJobs(courseId);
        if (active) {
          // Sort newest first
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-slate-500 dark:text-slate-400">Chưa có tiến trình tạo quiz nào.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {jobs.map((job) => (
                <div
                  ...
                  className="group flex items-center gap-4 p-4 rounded-xl border border-slate-200 dark:border-slate-800 hover:border-emerald-500 dark:hover:border-emerald-500 cursor-pointer transition-colors bg-white dark:bg-slate-900 hover:shadow-sm"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 text-sm text-slate-500">
                      ...
                      <span>•</span>
                      <span>{job.quizzes_count} quiz nháp</span>
                    </div>
                  </div>
                </div>
              ))}
